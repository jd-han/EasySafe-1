...
public class AppUserController {
	...
	// 검색 로그 입력하기
	@RequestMapping(value = "/searchLogInput.do", method = RequestMethod.POST)
	public void searchLogInput(@RequestBody KeywordVO kVO) {
		logger.info("input KeywordVO : " + kVO);
	...
	@ResponseBody
	@RequestMapping(value = "/searchLogOutput.do", method = RequestMethod.POST)
	public List<KeywordVO> searchLogOutput(@RequestBody KeywordVO kVO) {
		logger.info("input KeywordVO : " + kVO);
		return kService.selectKeywordByUser(kVO.getUser());
	}
	

	/*
	 * try { //외부로그인은 비번이 없다. if(voSub.getUpw() != null){ vo =
	 * service.getMemberByInfo(voSub.getUid()); //
	 * System.out.println(vo.getPassword()); //
	 * System.out.println(voSub.getPassword());
	 * if(vo.getUpw().equals(voSub.getUpw())){ System.out.println("패스워드 맞음"); //
	 * 일반 로그인 // 토큰 생성 tempToken = makeToken(vo); vo.setToken(tempToken); }else{
	 * System.out.println("패스워드 안맞음!!"); // 패스워드 안맞음. throw new Exception(); }
	 * // }else if(voSub.getFacebookId() != null || voSub.getGoogleId() !=
	 * null){ // // 외부 로그인 // // 토큰 생성 // vo =
	 * service.getMemberByInfo(voSub.getUid()); // tempToken = makeToken(vo); //
	 * vo.setToken(tempToken); }else if(voSub.getToken() != null){ // 토큰 로그인 -
	 * 자동 로그인 - app - 토큰 생성 안함. vo = service.getMemberByToken(voSub.getToken());
	 * if(TokenUtil.tokenVerify(voSub.getToken(), vo.getApikey())){ tempToken =
	 * voSub.getToken(); }else{ // 다시 로그인 해야함. mapMemberInfo.put("ourToken",
	 * "reLoginRequest"); } }else{ // 그외 던짐 throw new Exception(); }
	 * mapMemberInfo.put("ourToken", tempToken); // 로그인 세션 처리
	 * session.setAttribute("member", vo); return mapMemberInfo; } catch
	 * (Exception e) { e.printStackTrace(); mapMemberInfo.put("ourToken",
	 * "Fail"); return mapMemberInfo; <<<<<<< HEAD } }
	 * 
	 * // 로그인
	 * 
	 * @RequestMapping(value = "/login", method = RequestMethod.POST) public
	 * Map<String, Object> loginMember(@RequestBody UserVO vo, HttpSession
	 * session) { // 변형된 암호를 비교하기 위한 작업 if(vo.getUpw() != null){
	 * vo.setUpw(Sha512Encrypt.hash(vo.getUpw())); } mapMemberInfo =
	 * loginCommonMember(vo, session); return mapMemberInfo; } ======= }
	 */
	 ...
	 * ResponseEntity<String> entity = null; System.out.println(vo.toString());
	 * try { if(vo.getUpw() != null){
	 * vo.setUpw(Sha512Encrypt.hash(vo.getUpw())); }
	 * 
	 * service.modifyUser(vo);
	 * 
	 * entity = new ResponseEntity<String>("SUCCESS", HttpStatus.OK); } catch
	 * (Exception e) { e.printStackTrace(); } return entity; }
	 * 
	 * // 회원탈퇴
	 * 
	 * @RequestMapping(value = "/deleteUser.do", method = RequestMethod.POST)
	 * public ResponseEntity<String> deleteUser(@RequestBody UserVO vo) {
	 * System.out.println(vo.getUid()); ResponseEntity<String> entity = null;
	 * try { service.deleteUser(vo.getUid()); entity = new
	...
	// public void addInfo(MultipartHttpServletRequest mRequest,
	...
	// List<FileVO> fileList = fService.fileUpload(mRequest,
	// Integer.parseInt(mRequest.getParameter("parentNum")),
	// Integer.parseInt(mRequest.getParameter("parentType")));
	 ...
	 * public boolean confirmMember(UserVO vo){ boolean flag = false; String
	 * password = new String(); try { if(idChecker(vo.getUid())){ password =
	 * service.getUserById(vo.getUid()).getUpw(); if(password == null &&
	 * vo.getUpw().length()==0){ flag = true; }else{
	 * vo.setUpw(Sha512Encrypt.hash(vo.getUpw()));
	 * if(password.equals(vo.getUpw())){ flag = true; }else{ flag = false; } }
	 * }else{ flag = false; } } catch (Exception e) {
	 * System.out.println("confirmMember :::: 회원정보가 없음."); e.printStackTrace();
